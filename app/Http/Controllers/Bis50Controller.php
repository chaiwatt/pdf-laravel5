...
class Bis50Controller extends Controller
{
    ...
    public function addSignVerify()
    {
        $attach_path = 'pdf';
        $data_pdf = $this->verify('I');
        $type = 3;
    ...
    public function verify()
    {
        ...
        // สร้างข้อมูลภาพ QR Code
        $qrImage = QrCode::format('png')
            // ->merge(public_path('images/tisi.png'), 0.2, true)
            ->size(256)
            ->generate($url);
        ...
        $footer = view('pdf.scope-footer', [
            'qrImage' => $qrImage,
            'sign1Image' => $signImage,
            'sign2Image' => $sign1Image,
    ...
    public function addSign()
    {
        // สร้าง mpdf object
        // $mpdf = new Mpdf();  
        // $pagecount = $mpdf->SetSourceFile(public_path('pdf/scope.pdf'));  // เปิดไฟล์ PDF เดิม
    
        // // กำหนดตำแหน่งและขนาดของลายเซ็น
        // $signaturePath = public_path('images/sign.jpg');
        // $signatureWidth = 20;  // ความกว้างของลายเซ็น (ใน mm)
        // $signatureHeight = 10; // ความสูงของลายเซ็น (ใน mm)
        // $positionY = 265; // ตำแหน่ง Y (ใน mm) สำหรับด้านล่างของหน้า
    
        // // ดึงหน้าจาก PDF และแทรกลงใน mPDF
        // for ($i = 1; $i <= $pagecount; $i++) {
        //     $tplId = $mpdf->ImportPage($i); // ดึงหน้าแต่ละหน้าจาก PDF
        //     $mpdf->UseTemplate($tplId);     // แทรกหน้าจาก PDF เดิม
    
        //     // แทรกลายเซ็นในแต่ละหน้า (ที่ด้านล่างของหน้า)
        //     $mpdf->Image($signaturePath, 150, $positionY, $signatureWidth, $signatureHeight);
    
        //     // เพิ่มหน้าใหม่หลังจากที่เพิ่มลายเซ็นลงไปแล้ว
        //     // if ($i < $pagecount) {
        //     //     $mpdf->AddPage(); // เพิ่มหน้านี้หลังจากใช้หน้าเดิมแล้ว
        //     // }
        // }
    
        // // ส่งออกไฟล์ PDF ใหม่
        // $mpdf->Output('signed_document.pdf', 'I');


        $pdfFile = public_path('pdf/scope.pdf');
        $imageFile = public_path('images/sign.jpg');

        $mpdf = new Mpdf();

        // โหลดไฟล์ PDF ที่มีอยู่เดิม
        $pageCount = $mpdf->SetSourceFile($pdfFile);
        
        // ทำการเพิ่มหน้า PDF เดิมทั้งหมด
        for ($i = 1; $i <= $pageCount; $i++) {
            // นำหน้าจาก PDF เดิมมาใช้
            $tplId = $mpdf->ImportPage($i);
            // echo($imageFile);
            $mpdf->UseTemplate($tplId);

            // ในหน้าแรก เพิ่มรูปภาพที่ต้องการ เช่น ลงลายเซ็น
            // if ($i == 1) {
                $mpdf->Image($imageFile, 150, 240, 20, 10, 'jpg', '', true, false); // ระบุตำแหน่งและขนาดรูปภาพ
            // }

            // ไม่ต้องเรียก AddPage เพราะเราต้องการเพิ่มรูปบนหน้าเดิม
        }

        // บันทึกไฟล์ PDF ใหม่ที่มีการเพิ่มรูปภาพ
        $outputFile = public_path('pdf/modified_scope.pdf');
        $mpdf->Output($outputFile, Destination::FILE);
    }
    
    public function getRegister($file ='' ,$type ='' ,$certificate_no = '',$attach_path='pdf')
    {
